...
void AnticheatMgr::BuildReport(Player* player,uint8 reportType)
{
    ...
    if (m_Players[key].GetTotalReports() > sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION))
    {
        ...
        std::string str = "|cffff0000[YugoWoW-AntiCheat]|cFF60FF00 Stop Cheating!";
		if (sWorld->getBoolConfig(CONFIG_BAN_PLAYER)) //Make anticheat active.
        {
            if (m_Players[key].GetAverage() > 0.5f)
            {
                str = "|cffff0000[YugoWoW-AntiCheat]|cFF60FF00[" + std::string(player->GetName().c_str()) + "] |cFF00FFFFPossible cheater! -- |cff00ff00Banned for 10 min.";
				WorldPacket data(SMSG_NOTIFICATION, (str.size() + 1));
				sWorld->BanCharacter(player->GetName(), "10m", str, "Cheating");
				data << str;
        ...
        {
        str = "|cffff0000[YugoWoW-AntiCheat]|cFF60FF00[" + std::string(player->GetName().c_str()) + "] |cFF00FFFFPossible cheater! -- Warning Only";
        WorldPacket data(SMSG_NOTIFICATION, (str.size()+1));
        data << str;
        sWorld->SendGlobalGMMessage(&data);
		}
