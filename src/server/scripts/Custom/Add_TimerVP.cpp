...
#define EQUIRED_ONLINE_TIME 1800 // in seconds '3600' = 1hour
...
class Online_Time_VP : public WorldScript
{
public:
    Online_Time_VP() : WorldScript("Online_Time_VP") { }
			...
			for (SessionMap::const_iterator iter = smap.begin(); iter != smap.end(); ++iter)
			{
				if (Player* player = iter->second->GetPlayer())
				{
					...
					if(extention.online_time >= EQUIRED_ONLINE_TIME)
					{
						...
						LoginDatabase.PExecute("UPDATE `%s`.`%s` SET `vp`=`vp`+%u WHERE `id`='%u'", DATABASE, TABLE, REWARD_AMOUNT, player->GetSession()->GetAccountId());
						// now reset his online time
						m_PlayerExtention.erase(player_id);
						ChatHandler(player->GetSession()).PSendSysMessage("Congratulations you earned %u vp since you've been online for 30 min.", REWARD_AMOUNT);
					}
				}
			}
		}
