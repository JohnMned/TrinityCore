...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature)
	{
		...
		player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_2, "|TInterface\\icons\\Achievement_PVP_O_02:25|t[200 Kills] Show me what you got!", GOSSIP_SENDER_MAIN, 1002); //second option, click to send to the vendor (if he has 5k kills)
	...
	bool OnGossipSelect(Player* player, Creature* pCreature, uint32 /*uiSender*/, uint32 action)
	{
		...
		case 1002:
			if (player->GetUInt32Value(PLAYER_FIELD_LIFETIME_HONORABLE_KILLS) >= 200)
			{
				player->GetSession()->SendListInventory(pCreature->GetGUID()), //send to the vendor himself. add items via .npc add item command
					pCreature->Whisper("You have 200 kills or more, therefore you can view my goods.", LANG_UNIVERSAL, player); //if the player has 5k kills, send this message when 
			}
			else
			{
				pCreature->Whisper("You do not have 200 kills or more, therefore I cannot allow you to check my store. Sorry.", LANG_UNIVERSAL, player), player->CLOSE_GOSSIP_MENU(); //send this "error" message and close everything
			}
			break;
	}
		return true;
